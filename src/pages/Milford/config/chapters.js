const chapters = [
  {
    id: "slide-0",
    title: "🚶‍♂️ Milford Track 🇳🇿",
    alignment: "left",
    description: `Experience the stunning beauty of New Zealand's natural landscapes on the Milford Track, one of the world's greatest walks. Traverse through majestic mountains 🏔️, crystal-clear lakes 💦, and lush forests 🌳, while immersing yourself in the country's rich flora and fauna 🦜. <br> <br> 🚤 Before getting there you'll need to sort out transport to the start of the track ... The most popular way is catching a ferry from Te Anau Downs using companies such as <a href="https://www.fiordlandoutdoors.co.nz/transport-and-experiences/milford-track-transport/">Fiordland Outdoors</a>, <a href="https://www.tracknet.net/tracks/milford">Tracknet</a> or <a href="https://www.realnz.com/en/experiences/ferry-services/milford-track-transfers/">RealNZ</a>. These companies will also sort out your transport on the other side of the track and can include a bus back to Te Anau Downs, Te Anau or Queenstown.  <br> <br> 🥾 The most hardcore of the hikers can also consider taking the eight hour hike over <a href="https://www.doc.govt.nz/parks-and-recreation/places-to-go/fiordland/places/fiordland-national-park/things-to-do/tracks/dore-pass-route/">Dore Pass</a> 😱`,
    location: {},
    onChapterEnter: [],
    onChapterExit: [],
  },
  {
    id: "drive-slide-0",
    title: "🚣‍♀️ Day 1: Wetlands & Clinton Hut",
    description:
      "The first day of the Milford Track usually starts with a boat ride to Glade Wharf 🚤, where hikers can enjoy a peaceful setting surrounded by lush forest 🌳. From there, the Wetland Walk takes hikers through a serene marshland area before reaching Clinton Hut 🏕️, where they can rest for the night. 🌜 Make sure to talk a short wander to the glow worms 🐛 from Clinton Hut once the sun goes down. 🌅",
    alignment: "right",
    location: {},
    onChapterEnter: [],
    onChapterExit: [],
  },
  {
    id: "drive-slide-1",
    title: "Day 2: Arthur Valley",
    alignment: "left",
    description:
      "The second day of the Milford Track involves a hike through the Clinton Valley, passing by stunning waterfalls and traversing the valley floor. Take in the panoramic views of the surrounding mountains. There is a small detour worth taking to the Hidden Lake.",
    location: {},
    onChapterEnter: [],
    onChapterExit: [],
  },
  {
    id: "drive-slide-2",
    title: "Day 2: Prarie Shelter & Bus Stop Shelter",
    alignment: "right",
    description:
      "The Prairie Shelter is a small shelter located about 3 hours into the second day's hike. It provides a welcome rest stop for hikers, particularly in wet weather. The Bus Stop Shelter is another small shelter and is a must-stop if the rain is heavy or the river level is too high to cross safely, as it provides protection and shelter for hikers.",
    location: {},
    onChapterEnter: [],
    onChapterExit: [],
  },
  {
    id: "drive-slide-3",
    title: "Day 3: Mintaro Hut & Mackinnon Pass",
    alignment: "left",
    description:
      "Mintaro Hut provides stunning views of the surrounding mountains and is a great place to rest and refuel for the journey ahead. The Mackinnon Pass is the highest point on the track that offers breathtaking views of the surrounding mountain peaks and valleys. If hikers arrive early and the weather is clear, it is worth dropping off their packs at the hut and taking a side trip to the Mackinnon Pass Memorial, which honors the track's builder, Quintin Mackinnon. The memorial is often shrouded in clouds in the mornings, so hiking to it when the weather is clear is a rare opportunity to take in the stunning views from the pass.",
    location: {},
    onChapterEnter: [],
    onChapterExit: [],
  },
  {
    id: "drive-slide-4",
    title: "Day 3: Sutherland Falls",
    alignment: "left",
    description:
      "After reaching the summit of Mackinnon Pass, hikers will descend down towards the Quintin Shelter, where free hot water is available. From there, hikers can choose to take a side trip to Sutherland Falls, which involves a 1 hour detour that leads to the base of the falls. The falls are one of the highest in New Zealand, and the view from the base is well worth the extra effort.",
    location: {},
    onChapterEnter: [],
    onChapterExit: [],
  },
  {
    id: "drive-slide-5",
    title: "Day 4: Dumpling Hut",
    alignment: "right",
    description:
      "Dumpling Hut is the final hut on the Milford Track. A welcome rest after a long day offering 4 different bunk rooms.",
    location: {},
    onChapterEnter: [],
    onChapterExit: [],
  },
  {
    id: "drive-slide-6",
    title: "Day 4: Bell Rock & Mackay Falls",
    alignment: "left",
    description:
      "The final day is the longest but thankfully flat. Climbing into Bell Rock is a must and at the same time visiting Mackay Falls.",
    location: {},
    onChapterEnter: [],
    onChapterExit: [],
  },
  {
    id: "drive-slide-7",
    title: "Day 4: Lake Ada - Final Stretch",
    alignment: "right",
    description: "Final streettchh ayeee",
    location: {},
    onChapterEnter: [],
    onChapterExit: [],
  },
  {
    id: "slide-8",
    title: "Sandfly Point",
    alignment: "center",
    // image: './path/to/image/source.png',
    description: "Sandfly point",
    location: {},
    onChapterEnter: [],
    onChapterExit: [],
  },
];

export default chapters;
